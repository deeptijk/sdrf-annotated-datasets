...
See `manifest.tsv` for PRIDE URLs, Europe PMC hits, platform calls, primary matrix counts, skipped secondary files, sample row counts, and draft paths. See `column-pruning.tsv` for the per-accession fields removed by the all-`not available` cleanup, and `repetitive-column-removal.tsv` for the duplicate/overlapping optional fields removed. See `autoresearch-round1.tsv` for per-accession draft status and validation status. See `validation-round2.tsv` for per-template validation results.
