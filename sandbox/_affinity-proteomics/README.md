...
See `manifest.tsv` for PRIDE URLs, Europe PMC hits, platform calls, primary matrix counts, skipped secondary files, sample row counts, and draft paths. See `autoresearch-round1.tsv` for per-accession draft status and validation status. See `validation-round2.tsv` for per-template validation results.
